...
- ``rule_based``    — THE agent this repository exists for: a SelectContext
                      dispatch table with an outermost safety guard that always
                      falls back to a legal random selection. Rules are added as
                      pure per-context functions without touching the guard.
"""
